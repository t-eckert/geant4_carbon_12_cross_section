...
{ 
	 ...
	 /*//Epoxy Plug
	 EPOuterRadius = 5.0*mm/2;
	 EPHalfHeight = 3.18*mm/2;
	 EPOffset = -0.405*mm;*/

	 //Acrylic Holder
	 AcHInnerRadius = 0.0*mm/2;
	 AcHOuterRadius = 2.54*cm/2;
	 AcHHalfHeight = 6.35*mm/2;
	 AcHOffset = -0.405*mm;
	 ...
	 //Copper Disk 1
	 CD1OuterRadius = 2.54*cm/2;
	 CD1HalfHeight = 0.1588*cm/2;
	 CD1Offset = CD1HalfHeight +GetSourceZoffset() +AcHOffset +AcHHalfHeight;

	 //Copper Disk 2
	 CD2OuterRadius = 2.54*cm/2;
	 CD2HalfHeight = 0.1588*cm/2;
	 CD2Offset = -CD2HalfHeight -GetSourceZoffset() +AcHOffset -AcHHalfHeight;
...
/*
 ...
 //===============BEGIN=Acrylic Holder========================//

   G4Tubs *AcHolder = new G4Tubs("Acrylic Holder", AcHInnerRadius, AcHOuterRadius, AcHHalfHeight, 0.0, 360.*deg);
   AcH_log = new G4LogicalVolume(AcHolder, Acrylic, "AcH_log", 0, 0, 0);
   AcH_phys = new G4PVPlacement(0, G4ThreeVector(xChange, yChange, AcHOffset), AcH_log, "Acrylic Holder", Room_log, false, 0);
   G4VisAttributes AcH_Att(G4Color::Gray());
   AcH_Att.SetForceWireframe(false);
   AcH_Att.SetForceSolid(true);
   AcH_Att.SetVisibility(true);
   AcH_log -> SetVisAttributes (AcH_Att);

 //===============END===Acrylic Holder========================//*/

 /*//===============BEGIN=Epoxy Plug========================//

   G4Tubs *EpoxyPlug = new G4Tubs("Epoxy Plug", 0.0, EPOuterRadius, EPHalfHeight, 0.0, 360.*deg);
   EP_log = new G4LogicalVolume(EpoxyPlug, Epoxy, "EP_log", 0, 0, 0);
   EP_phys = new G4PVPlacement(0, G4ThreeVector(xChange, yChange, EPOffset), EP_log, "Epoxy Plug", Room_log, false, 0);
   G4VisAttributes EP_Att(G4Color::Blue());
   EP_Att.SetForceWireframe(false);
   EP_Att.SetForceSolid(true);
   EP_Att.SetVisibility(true);
   EP_log -> SetVisAttributes (EP_Att);
 ...
 // BEGIN NaI
  ...
  NaI1Reflector_phys = new G4PVPlacement(0, G4ThreeVector(x_pos, y_pos, z_pos), NaI1Reflector_log,"NaI1Reflector",Room_log,false,0);
  
  z_pos = 0 -z_pos;
  NaI2Reflector_phys = new G4PVPlacement(0, G4ThreeVector(x_pos, y_pos, z_pos), NaI2Reflector_log,"NaI2Reflector",Room_log,false,0);


  G4VisAttributes NaIReflector_Att(G4Color::Green());
  NaIReflector_Att.SetVisibility(true);
  NaIReflector_Att.SetForceWireframe(false);
...
// ------ END REFLECTOR ------
 ...
 NaI2Wall_phys = new G4PVPlacement(pRot, G4ThreeVector(x_pos, y_pos, z_pos), NaI2Wall_log, "Na2Wall", Room_log, false, 0);
  ...
  NaI1_Att.SetForceWireframe(false);
  ...
  NaIWall_Att.SetForceWireframe(false);
