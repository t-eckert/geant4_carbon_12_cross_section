...
{ 
	 ...
	 NaIWallThickness = 0.508*mm;				// Al wall of NaI detctor //Check that this is correct.


	 SetSourceInnerRadius(1.746*cm/2);		// Source inner radius
	 SetSourceRadius(7.62/2*cm);			// Source outer radius
	 SetSourceHalfHeight(0.89*cm/2);		// Half of source height	
	 SetSourceXoffset(0.0*cm);				// Source center position
	 SetSourceYoffset(0.0*cm);
	 SetSourceZoffset(0.0*cm);
	 SetMaterialName("Graphite");

	 NaIReflectorThickness = 2.8*mm;

	 SetDetectorGap(0.89*cm);		// gap between detectors. Can be overwritten in batch.mac;
	;}
