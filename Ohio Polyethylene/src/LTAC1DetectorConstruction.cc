...
{ 
	 ...
	 SetSourceRadius(2.54/2*cm);				// Source outer radius
	 SetSourceHalfHeight(0.164*cm/2);			// Half of source height	
	 SetSourceXoffset(0.0*cm);				// Source center position
	 SetSourceYoffset(0.0*cm);
	 SetSourceZoffset(0.0*cm);
	 SetMaterialName("Polyethylene");

	 NaIReflectorThickness = 2.7*mm;

	 SetDetectorGap(0.164*cm);		// gap between detectors. Can be overwritten in batch.mac;
	;}
...
G4VPhysicalVolume* LTAC1DetectorConstruction::Construct()
{
 ...
 //Graphite->AddElement(elC,1);
  ...
  G4Material *Polyethylene = new G4Material("Polyethylene", density = 0.96*g/cm3, ncomponents = 2 );
...
//
 ...
 //---------------BEGIN--- Polyethylene Source ----------------------//
  ...
  Source_log = new G4LogicalVolume(Source, Polyethylene, "Source_log", 0, 0, 0);
